...
local maxItems = 10
local maxPages = math.ceil(#packtable/maxItems)
local curPage = 1
...
local function packList()
	...
	local frameHeight = SCREEN_HEIGHT - 60
	local verts= {
		{{-30, 0, 0}, getMainColor('highlight')},
		{{-15, 20, 0}, getMainColor('highlight')},
		{{0, 0, 0}, getMainColor('highlight')},
	}
	...
	t[#t+1] = Triangle(verts) .. {
		InitCommand=function(self)
			self:xy(110,frameHeight-10)
			self:diffusealpha(0.8)
			self:rotationz(90)
		end;
	};	
	...
	t[#t+1] = Triangle(verts) .. {
		InitCommand=function(self)
			self:xy(frameWidth-110,frameHeight-40)
			self:diffusealpha(0.8)
			self:rotationz(270)
		end;
	};
	...
	local function packItem(i)
		...
		local t = Def.ActorFrame{
			...
			UpdateListMessageCommand = function(self) -- Pack List updates (e.g. new page)
				...
				if packtable[packIndex] ~= nil then
			...
			DownloadStatusCommand = function(self, params) -- Download status update from updatefunction
				...
				if params.pack == packtable[packIndex] then
			...
			DownloadFailedMessageCommand = function(self, params) -- Download Failed
				if packtable[packIndex]:GetName() == params.pack:GetName() then 
					downloading = DLMAN:GetDownloadingPacks()
					self:GetChild("Status"):playcommand("Set")
					self:GetChild("ProgressBar"):diffuse(color(colorConfig:get_data().downloadStatus.available)):diffusealpha(0.2)
		...
		t[#t+1] = quadButton(6) .. {
			...
			TopPressedCommand = function(self)
				if packtable[packIndex]:IsDownloading() then -- IsDownloading() returns the wrong boolean for some reason.
		...
		t[#t+1] = Def.Quad{
			...
			SetCommand = function(self)
				if packExists(packtable[packIndex]:GetName()) then
		...
		t[#t+1] = LoadFont("Common Bold")..{
			...
			SetCommand = function(self)
				local msd = packtable[packIndex]:GetAvgDifficulty()
		...
		t[#t+1] = LoadFont("Common Bold")..{
			...
			SetCommand = function(self)
				self:settextf("%s",packtable[packIndex]:GetName())
				self:maxwidth(2*packItemWidth)
			end
		}
		...
		t[#t+1] = LoadFont("Common Normal")..{
			...
			SetCommand = function(self)
				if packExists(packtable[packIndex]:GetName()) then
					...
					self:settextf("Download %5.2f MB",packtable[packIndex]:GetSize()/1048576)
				end
			end
		}
